 ...
 * Time: 15:58:31
 */
public class SoapTranslatorTest extends TestCase
{
	public SoapTranslatorTest(final String name)
	{
		super(name);
	}
  ...
  public void testEncodeRequest()
  {
    ...
    try
    {
      final String encoded = new SoapCoder().encode(r);
  ...
  public void testDecodeResponse()
  {
    try
    {
      final String content = TestUtils.readFile("test/samples/soap.response.xml");

      final Response r = new SoapCoder().decode(content);
      assertNotNull(r);
      ...
      assertEquals(2517, req.getTransactionTag());
  ...
  public void testDecodeDeserializationFault()
  {
    try
    {
      final String content = TestUtils.readFile("test/samples/soap.deserialization.fault.xml");

      final Response r = new SoapCoder().decode(content);
      fail("CoderException expected");
    }
    catch (IOException e)
    {
      e.printStackTrace();
    ...
    {
      assertEquals("Server was unable to read request. --> There is an error in XML document (1, 406). --> <SendAndCommit xmlns='http://secure2.exact.com/vplug-in/transaction/rpc-enc/'> was not expected.", e.getMessage());
    }

  }

  public void testDecodeGeneralFailureFault() throws IOException
  {
    try
    {
      final String content = TestUtils.readFile("test/samples/soap.deserialization.fault.xml");
  ...
  public void testDecodeNullTransactionFault()
  {
    try
    {
      final String content = TestUtils.readFile("test/samples/soap.nulltransaction.fault.xml");
      ...
      assertEquals("Bad Request. Object reference not set to an instance of a Transaction object.", r.getErrorDescription());
    }
    catch (Exception e)
    {
      e.printStackTrace();
  ...
  public void testDecodesDodgyResponse()
  {
    try
    {
      final String content = TestUtils.readFile("test/samples/soap.dodgy.response.xml");
      ...
      assertEquals("4111111111111111666666", req.getCardNumber());
      assertEquals(2517, req.getTransactionTag());
    }
    catch (Exception e)
    {
  ...
  public void testEverythingResponse()
	{
		try
		{
      final String content = TestUtils.readFile("test/samples/soap.everything.response.xml");
