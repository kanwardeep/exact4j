 ...
 * Time: 16:54:03
 */
public class TaggedPreAuthCompletionTest extends BaseTestCase
{
  private Response r = null;

  public TaggedPreAuthCompletionTest(final String name) {
    super(name);
  }
  ...
  public void testMandatory() {
    final Request request = new Request(TransactionType.TaggedPreAuthCompletion);
  ...
  public void testJsonWorks() {
    final Request request = getCredentialledRequest(TransactionType.TaggedPreAuthCompletion);
  ...
  public void testMultiplePurchasesAgainstOriginalSeed() {
    final Request request = getCredentialledRequest(TransactionType.TaggedPreAuthCompletion);
    request.setTransactionTag(r.getRequest().getTransactionTag());
    request.setAuthorizationNum(r.getRequest().getAuthorizationNum());
    ...
    // can't complete an already completed txn
  ...
  public void testNotPermittedAfterRecurringSeedPurchase() {
    final Response r = doRecurringSeedPurchase();
